...
const DocumentItem = ({ documentId }: { documentId: number }) => {
  return (
    <Link href={`/listaDocumentos/${documentId}`} passHref legacyBehavior>
      <a className="flex flex-col items-center p-4 border border-gray-200 rounded-md mr-2 mb-4 hover:bg-gray-100 hover:bg-opacity-50 transition duration-300">
      ...
      {documentRows.map((row: any[], rowIndex: number) => (
        <div key={rowIndex} className="flex mb-4">
          {row.map((_, colIndex: number) => (
            <DocumentItem key={colIndex} documentId={rowIndex * 4 + colIndex} />
          ))}
        </div>
      ))}
    </div>
